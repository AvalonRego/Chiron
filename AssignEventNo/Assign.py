...
def balance_event_types(hits: pd.DataFrame, records: pd.DataFrame) -> (pd.DataFrame, pd.DataFrame):
    ...
    count_1s = (records['truth'] == 1).sum()
    count_0s = (records['truth'] == 0).sum()
    ...
    if count_1s > upper_limit:
        ...
        discard_event_nos = np.random.choice(records.loc[records['truth'] == 1, 'event_no'], excess, replace=False)
...
def process_and_save_hdf5(original_file_path,time_interval):
    ...
    try:
        ...
        temp_dir = f'{output_dir}_EN_{time_interval}_with_type' # Start with the original output_dir
        i = 1
        ...
        with pd.HDFStore(modified_file_path, mode='a') as store:  # 'a' mode allows modifying the file
            ...
            event_truth['truth'] = (event_truth['type'] // 19).astype(int)
...
if __name__ == "__main__":
    ...
    process_files_parallel(files, time_interval=100,num_workers=32)  # Adjust num_workers based on system capacity
