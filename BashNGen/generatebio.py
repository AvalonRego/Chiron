...
def larger_dataset_gen(seed):
    try:
        ...
        noise_generator_config = BioluminescenceGeneratorConfiguration(
        type=NoiseType.BIOLUMINESCENCE,
        start_time=0,
        duration=4_000,
        julia_data_path='/u/arego/Project/biolumi_sims',
        batch_size= 48
        )
...
def get_unique_path(seed):
    ...
    path = f'/viper/ptmp/arego/LargeBio/4000s/{seed}.h5'
    print(path)
    initialseed=seed
    
    # Check if the file exists
    if os.path.exists(path):
        while os.path.exists(f'/viper/ptmp/arego/LargeBio/4000s/{seed}.h5'):
            seed += 1
