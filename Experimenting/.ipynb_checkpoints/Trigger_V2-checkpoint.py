...
class TriggerDatasetProcessor:
    ...
    def prepare_trigger_data(self, hit):
        ...
        for i, j in zip(self.times, self.times[1:]):
            # Calculate hit counts for each interval and store in a new DataFrame
            interval_col = hit['time'].apply(lambda x: self.count_in_interval(x, i, j))
            interval_data.append(interval_col.rename(f'hit_in_interval_{i}-{j}'))

        # Concatenate all interval columns into the hit DataFrame at once
