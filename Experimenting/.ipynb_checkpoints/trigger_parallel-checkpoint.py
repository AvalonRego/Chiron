...
class TriggerDatasetProcessor:
    ...
    def prepare_trigger_data(self, hit):
        ...
        for i, j in zip(self.times, self.times[1:]):
            hit_in_intervals[f'hit_in_interval_{i}-{j}'] = hit['time'].apply(lambda x: self.count_in_interval(x, i, j))

        # Create a DataFrame from the dictionary
        for key, value in hit_in_intervals.items():
            hit[key] = value
    ...
    def process_record(self, record):
        ...
        # hit = self.hits.df.copy() if self.ignore_records else self.hits.get_by_record_ids(record).df.copy()
        # hit = hit.groupby(["string_id", 'module_id', 'pmt_id'])['time'].apply(list).reset_index()

        # import pickle
        # x=pickle.dumps(self.prepare_trigger_data)
        # print('dumped')
        ...
        return record #[number_of_modules_triggered, hits_per_interval, StackedTriggerData['time']]
    ...
    def process_records(self):
        ...
        _=Parallel(n_jobs=2,verbose=50)(delayed(self.process_record)(record) for record in self.records)

        
        
        return _ #NumberOfModsTriggeredPerRec, HitsPerIntervalPerRec, HitTiming
