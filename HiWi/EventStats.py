...
def extract_record_metrics(hits_chunk: pd.DataFrame, record_times_dict: Dict, record_hit_counts_dict: Dict) -> None:
    time_stats = hits_chunk.groupby('event_no')['time'].agg(['min', 'max', 'count'])
    for event_no, row in time_stats.iterrows():
        min_time, max_time, count = row['min'], row['max'], row['count']
        if event_no in record_hit_counts_dict:
            record_hit_counts_dict[event_no] += count
        else:
            record_hit_counts_dict[event_no] = count
        if event_no in record_times_dict:
            current_min, current_max = record_times_dict[event_no]
            record_times_dict[event_no] = [min(current_min, min_time), max(current_max, max_time)]
        else:
            record_times_dict[event_no] = [min_time, max_time]


def finalize_record_metrics(file_id: int, record_times_dict: Dict, record_hit_counts_dict: Dict, records_df: pd.DataFrame) -> pd.DataFrame:
    """
    Modified to include energy from records_df
    """
    results = []
    
    # Create a lookup dictionary for energy values
    energy_lookup = records_df.set_index('event_no')['energy'].to_dict() if 'energy' in records_df.columns else {}
    
    for event_no in record_times_dict.keys():
        min_time, max_time = record_times_dict[event_no]
        hit_count = record_hit_counts_dict[event_no]
        duration = max_time - min_time
        energy = energy_lookup.get(event_no, None)  # Get energy or None if not found
...
def process_single_file(file_path: str, intermediate_dir: str) -> bool:
    try:
        ...
        for start_idx in range(0, total_hits, chunk_size):
            ...
            extract_record_metrics(hits_chunk, record_times_dict, record_hit_counts_dict)
            del hits_chunk
            gc.collect()
        
        # Pass records_df to finalize_record_metrics
        final_df = finalize_record_metrics(file_id, record_times_dict, record_hit_counts_dict, records_df)
...
def merge_intermediate_files(intermediate_dir: str, output_path: str, expected_files: List[str]) -> bool:
    ...
    try:
        ...
        final_df = final_df.sort_values(['file_id', 'event_no']).reset_index(drop=True)
