...
# Path to the first Bash script (Starting_Inc.sh)
script1_path="Starting_Inc.sh"  # Update with the correct path if needed

# Path to the Python script directory
python_script_dir="../Experimenting/GenerationFiles"  # Directory where your Python scripts are located

# Manual step: Set the Python script you want to run here
# Example: python_script="generatestarting.py"
# Change this variable to the name of the Python script you want to run for each iteration.
python_script="generatestart.py"  # Set the name of your Python script here (e.g., generatestarting.py, another_script.py)
...
for (( i=1; i<=total_runs; i++ )); do
    ...
    # Step 1: Run the first Bash script (Starting_Inc.sh)
    echo "Running Starting_Inc.sh..."
    if [ -f "$script1_path" ]; then
        # If the Bash script exists, run it
        bash "$script1_path"
        if [ $? -ne 0 ]; then
            # If the Bash script fails, exit with an error
            echo "Error: Starting_Inc.sh failed. Stopping the process."
            exit 1
        fi
    else
        # If the Bash script doesn't exist, show an error and exit
        echo "Error: Starting_Inc.sh not found. Stopping the process."
        exit 1
    fi
