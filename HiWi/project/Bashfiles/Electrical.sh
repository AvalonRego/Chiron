...
script1_path="/u/arego/project/Bashfiles/Electrical_Inc.sh"  # Update with the correct path if needed
...
python_script="generatenoise.py"  # Set the name of your Python script here (e.g., generatecascade.py, another_script.py)
...
for (( i=1; i<=total_runs; i++ )); do
    ...
    if [ -f "$script1_path" ]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            echo "Error: Electrical_Inc.sh failed. Stopping the process."
            exit 1
        fi
    else
