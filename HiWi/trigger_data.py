...
def process_file(i, file_path, OUTPUT_DIR):
    ...
    try:
        ...
        for subset, plot_df in filtered_subset_data.items():
            ...
            # Option 1: Individual files per HDF5 file (safer, easier to debug)
            individual_output_path = os.path.join(subset_dir, f'{base_filename}.parquet')
            
            print(f"[{i}] Saving subset {subset_name} to {individual_output_path}")
            print(f"[{i}]   Data shape: {plot_df.shape}")
            
            # Save individual file (no locking needed since each file has unique name)
            table = pa.Table.from_pandas(plot_df)
            #pq.write_table(table, individual_output_path, compression=COMPRESSION)
            
            
            # Option 2: Also update merged file (with proper locking)
            merged_output_path = os.path.join(subset_dir, 'merged.parquet')
            lock_path = merged_output_path + ".lock"
            
            with FileLock(lock_path, timeout=3600):
                if os.path.exists(merged_output_path):
                    print(f"[{i}]   Appending to merged file")
                    existing_table = pq.read_table(merged_output_path)
                    combined_table = pa.concat_tables([existing_table, table])
                    pq.write_table(combined_table, merged_output_path, compression=COMPRESSION)
                    saved_files += 1
                else:
                    print(f"[{i}]   Creating new merged file")
                    pq.write_table(table, merged_output_path, compression=COMPRESSION)
                    saved_files += 1
        
        print(f"[{i}] Successfully saved {saved_files} subset files")
    except Exception as e:
        print(f"[{i}] ERROR saving data for {file_path} to {individual_output_path}: {e}")
        return False
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR+='/'+str(args.job_index + 1)
