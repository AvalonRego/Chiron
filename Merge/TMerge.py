...
def merger(file, base_path):
    ...
    try:
        ...
        # Define paths
        data_path = f"{file}"
        en_path = f"{base_path}LargeElectrical/7000s/{name}"
        bn_path = f"{base_path}LargeBio/7000s/{name}"
        save_path = f"{base_path}TMerge7k_1/{name}"
...
if __name__ == "__main__":
    ...
    process_files(files, base_path="/viper/ptmp/arego/", num_workers=16)
