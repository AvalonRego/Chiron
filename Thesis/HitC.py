...
def process_directory(directory, num_workers=8):
    ...
    return np.vstack(results) if results else np.empty((0, 1))

if __name__=="__main__":
# Example usage
    directory = "/viper/ptmp/arego/LargeElectrical/4000s"
    merged_array = process_directory(directory, num_workers=16)
    
    np.save("ELHitCount.npz",merged_array)
